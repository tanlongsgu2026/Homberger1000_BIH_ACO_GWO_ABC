...
from algorithms import GWOConfig, solve_gwo


def main():
    parser = argparse.ArgumentParser(description="Run GWO on Homberger 1000 VRPTW instances")
    ...
    config = GWOConfig(population_size=args.wolves, max_iterations=args.iterations, runtime_limit=args.runtime_limit, random_seed=args.seed, local_search_trials=args.local_search_trials)
    results = []
    for k, inst in enumerate(instances, start=1):
        print(f"\n[{k}/{len(instances)}] GWO solving {inst.name} ...")
        sol = solve_gwo(inst, config)
        results.append(sol)
    ...
    export_results_csv(results, Path(f"outputs/results/gwo_results_seed_{args.seed}.csv"))
    export_routes_csv(results, Path(f"outputs/routes/gwo_routes_seed_{args.seed}.csv"))
    export_convergence_csv(results, Path(f"outputs/convergence/gwo_convergence_seed_{args.seed}.csv"))
    print("\nSaved GWO outputs.")
